...
[ -x "${MSGATTRIB}" ] || exit ${LINENO}

MSGMERGE=$(which msgmerge 2>/dev/null)
[ -x "${MSGMERGE}" ] || exit ${LINENO}

LINGUIST_QT4=$(which linguist-qt4 2>/dev/null)
LINGUIST_COMMON=$(which linguist 2>/dev/null)
LINGUIST=${LINGUIST_COMMON:-${LINGUIST_QT4}}
[ -x "${LINGUIST}" ] || exit ${LINENO}
...
translate()
{
    ...
    if [[ -f ${output} ]]; then
        ...
        ${CAT} ${temp2} | ${SED} "s/THE PACKAGE'S COPYRIGHT HOLDER/PKB RIO/g" | ${SED} "s/PACKAGE/${target_name}/g" | ${SED} "s/VERSION/${VERSION}/g" > ${output}
    fi
